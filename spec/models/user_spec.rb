...
RSpec.describe User, type: :model do
  ...
  context '新規登録がうまくいかないとき' do
    ...
    it 'familyname_furiganaがひらがなだと保存できないこと' do
      @user.familyname_furigana = "たなか"
      @user.valid?
      expect(@user.errors.full_messages).to include("Familyname furigana is invalid")
    end

    it 'familyname_furiganaが漢字だと保存できないこと' do
      @user.familyname_furigana = "田中"
      @user.valid?
      expect(@user.errors.full_messages).to include("Familyname furigana is invalid")
    end
    ...
    it "name_furiganaが全角カナでないと保存できないこと" do
      @user.name_furigana = "taro"
      @user.valid?
      expect(@user.errors.full_messages).to include("Name furigana is invalid")
    end

    it "name_furiganaが全角ひらがなだと保存できないこと" do
      @user.name_furigana = "たろう"
      @user.valid?
      expect(@user.errors.full_messages).to include("Name furigana is invalid")
    end

    it "name_furiganaが漢字だと保存できないこと" do
